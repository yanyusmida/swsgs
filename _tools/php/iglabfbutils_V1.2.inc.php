<?php //required for IE in iframe FB environments if sessions are to work.
header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
/**
	Facebook Graph API Utils V.1.1
	Ignite Lab
	Author : Xerxis Anthony B. Alvar	
	Change Logs :
		[20 - 12 - 2011 ] :: Critical bug on "authorize_app", added urlencode on next url
		[16 - 12 - 2011 ] :: Modified function isFan => Added checking from Signed request
		[ 9 - 12 - 2011 ] :: Fix Session handling for signed_request,access_token,user_id
*/
...
abstract class iglabFbUtils {
	...
	public function isFan($user_fbid = null,$fanPageId = null) {
			//try checking the signed request first
			if (!empty($this->signed_request['user_id'])) {
				return true;
			} else {
				//try via FQL	
	...
	public function get_user_id() {
		...
		if (isset($_SESSION['user_id_'.$app_id])) {
			return $_SESSION['user_id_'.$app_id];
		} else {
			return false;
		}
	}
	...
	public function get_signed_request() {
		if (!$this->signed_request) {
		  ...
		  //find it in $_SESSION
			...
			if(!isset($_SESSION)) session_start();
			if (isset($_SESSION['signed_request_'.$app_id])) {
				$this->signed_request = $this->parseSignedRequest($_SESSION['signed_request_'.$app_id]);
		  	}// else {
	...
	*/
	public function get_user_access_token($signedRequest) {
		...
		if (!isset($_SESSION)) session_start();
		if (isset($_SESSION['access_token_'.$app_id]))  {
			//check if we still have a valid access token
			$this->validate_session_token($_SESSION['access_token_'.$app_id]);
		} else {
			return false;
		}
	...
	*/
	private function validate_session_token($access_token) {
	
		if (!empty($access_token) && isset($access_token) ) {
			
			$graph = "https://graph.facebook.com/kaixersoft?access_token=".$access_token;
			
			$res = json_decode(@file_get_contents($graph));
	...
	public function single_fql($sql) {
		...
		if ($accessToken != false) {
			...
			if (!$res['errno']) {
				...
				return (count($data['data'])) ? $data['data']: array();
			} else {
	...
	public function authorize_app($nextUrl,$perms) {
		if (!$this->isAppUser()) {		
			$oauthUrl = "http://www.facebook.com/dialog/oauth/?"
		  			  ...
		  			  ."&redirect_uri=".urlencode($nextUrl);
		  			 // ."&response_type=code";	 
